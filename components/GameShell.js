...
export default function GameShell() {
  ...
  useEffect(() => {
    ...
    function shoot() { if (player.cooldown > 0 || transition > 0) return; bullets.push({ x: player.x + (player.facing > 0 ? player.w : -14), y: player.y + 25, w: 14, h: 4, vx: player.facing * 720, damage: 1 }); player.cooldown = .13; audio.shot() }
    ...
    function damagePlayer() { if (player.invuln > 0 || transition > 0) return; localLives--; setLives(localLives); player.invuln = 1.4; burst(player.x + 17, player.y + 28, 18); audio.damage(); if (localLives <= 0) { setStatus('failed'); setStarted(false); saveScore(localScore); audio.gameOver(); return } player.x = Math.max(80, checkpoint + 120); player.y = 390; player.vx = 0; player.vy = 0; camera = Math.max(0, checkpoint); enemies.splice(0); enemyBullets.splice(0); waveCursor = Math.max(waveCursor, checkpoint) }
    function nextLevel() { localScore += 1500 + levelIndex * 500; setScore(localScore); if (levelIndex === LEVELS.length - 1) { setStatus('victory'); setStarted(false); saveScore(localScore); audio.victory(); return } levelIndex++; current = LEVELS[levelIndex]; setLevel(levelIndex + 1); camera = 0; checkpoint = 0; waveCursor = 0; bossSpawned = false; boss = null; bullets.splice(0); enemyBullets.splice(0); enemies.splice(0); pickups.splice(0); player.x = 120; player.y = 390; player.vx = 0; player.vy = 0; player.invuln = 1.4; transition = 2.2; audio.level() }
