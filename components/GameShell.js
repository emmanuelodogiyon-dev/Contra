...
export default function GameShell() {
  ...
  useEffect(() => {
    ...
    function burst(x, y, count = 12) {
      for (let i = 0; i < count; i++) particles.push({ x, y, vx: (Math.random() - .5) * 260, vy: (Math.random() - .8) * 260, t: .45 + Math.random() * .65 })
    }

    function shoot() {
      if (player.cooldown > 0 || transition > 0) return
      bullets.push({ x: player.x + (player.facing > 0 ? player.w : -14), y: player.y + 25, w: 14, h: 4, vx: player.facing * 720, damage: 1 })
      player.cooldown = .13
    }

    function enemyShot(e, speed = 260) {
      const dx = (player.x + player.w / 2) - (e.x + e.w / 2), dy = (player.y + player.h / 2) - (e.y + e.h / 2)
      const d = Math.hypot(dx, dy) || 1
      enemyBullets.push({ x: e.x + e.w / 2, y: e.y + e.h / 2, w: 8, h: 8, vx: dx / d * speed, vy: dy / d * speed })
    }

    function spawnEnemy(type, x) {
      const base = { type, x, y: 420, w: 34, h: 48, hp: 2, vx: -70, cooldown: .5 + Math.random(), bob: Math.random() * 6 }
      if (type === 'runner') Object.assign(base, { hp: 1, vx: -150, w: 30, h: 42, y: 426 })
      if (type === 'turret') Object.assign(base, { hp: 4, vx: 0, w: 40, h: 36, y: 432, cooldown: .4 })
      if (type === 'heavy') Object.assign(base, { hp: 7, vx: -42, w: 48, h: 58, y: 410, cooldown: .9 })
      enemies.push(base)
    }
    ...
    function spawnWave(mark) {
      ...
      spawnEnemy('grunt', x)
      if (difficulty >= 2 || mark > current.length * .3) spawnEnemy('runner', x + 180)
      if (mark % 900 < 250) spawnEnemy('turret', x + 330)
      if (difficulty >= 3 && mark > current.length * .45) spawnEnemy('heavy', x + 520)
      if (difficulty >= 5) spawnEnemy(Math.random() > .5 ? 'runner' : 'grunt', x + 700)
    }

    function spawnBoss() {
      bossSpawned = true
      const hp = 28 + levelIndex * 10
      boss = { x: current.length + 520, y: 340, w: 118, h: 128, hp, maxHp: hp, cooldown: 1, phase: 1, vx: 0, alive: true }
    }

    function dropPickup(x, y) {
      if (Math.random() < .24) pickups.push({ x, y, w: 22, h: 22, type: Math.random() < .7 ? 'medal' : 'life', vy: -120 })
    }

    function damagePlayer() {
      if (player.invuln > 0 || transition > 0) return
      localLives--
      setLives(localLives)
      player.invuln = 1.4
      burst(player.x + 17, player.y + 28, 18)
      if (localLives <= 0) {
        setStatus('failed')
        setStarted(false)
      ...
      player.x = Math.max(80, checkpoint + 120)
      player.y = 390; player.vx = 0; player.vy = 0
      camera = Math.max(0, checkpoint)
      enemies.splice(0); enemyBullets.splice(0)
      waveCursor = Math.max(waveCursor, checkpoint)
    }

    function nextLevel() {
      localScore += 1500 + levelIndex * 500
      setScore(localScore)
      ...
      camera = 0; checkpoint = 0; waveCursor = 0; bossSpawned = false; boss = null
      bullets.splice(0); enemyBullets.splice(0); enemies.splice(0); pickups.splice(0)
      player.x = 120; player.y = 390; player.vx = 0; player.vy = 0; player.invuln = 1.4
      transition = 2.2
    }
    ...
    function update(dt) {
      ...
      player.vx = transition > 0 ? 0 : (right - left) * 245
      ...
      player.vy += 1120 * dt; player.x += player.vx * dt; player.y += player.vy * dt
      ...
      if (!boss?.alive && player.x - camera > W * .56) camera = Math.min(current.length + 280, player.x - W * .56)
      if (boss?.alive && player.x - camera > W * .48) camera = Math.min(current.length + 260, player.x - W * .48)
      ...
      pickups.forEach(p => { p.vy += 520 * dt; p.y += p.vy * dt; if (p.y + p.h > GROUND) { p.y = GROUND - p.h; p.vy = 0 } })

      enemies.forEach(e => {
        if (e.type !== 'turret') e.x += e.vx * dt
        e.cooldown -= dt
        if ((e.type === 'turret' || e.type === 'heavy' || levelIndex >= 2) && e.cooldown <= 0 && Math.abs(e.x - player.x) < 620) {
          enemyShot(e, e.type === 'heavy' ? 300 : 245 + levelIndex * 12)
          e.cooldown = e.type === 'turret' ? 1.25 : e.type === 'heavy' ? 1.7 : 2.1
        }
        if (e.type === 'grunt' || e.type === 'runner') e.y = (e.type === 'runner' ? 426 : 420) + Math.sin(gameTime * 5 + e.bob) * 1.4
        if (e.x < camera - 120) e.hp = 0
      })

      if (boss?.alive) {
        boss.phase = boss.hp < boss.maxHp * .34 ? 3 : boss.hp < boss.maxHp * .67 ? 2 : 1
        boss.cooldown -= dt
        boss.y = 326 + Math.sin(gameTime * (1.5 + boss.phase * .35)) * (10 + boss.phase * 4)
        boss.x = current.length + 470 + Math.sin(gameTime * .8) * 52
        if (boss.cooldown <= 0) {
          const shots = boss.phase
          for (let i = 0; i < shots; i++) {
            const fake = { x: boss.x, y: boss.y + 32 + i * 25, w: boss.w, h: 20 }
            enemyShot(fake, 275 + levelIndex * 14 + boss.phase * 18)
          }
          boss.cooldown = 1.15 - boss.phase * .13
        }
      }

      for (const b of bullets) {
        for (const e of enemies) if (!b.dead && e.hp > 0 && hit(b, e)) {
          e.hp -= b.damage; b.dead = true; burst(b.x, b.y, 5)
          if (e.hp <= 0) { localScore += e.type === 'heavy' ? 350 : e.type === 'turret' ? 250 : e.type === 'runner' ? 150 : 100; setScore(localScore); dropPickup(e.x, e.y) }
        }
        if (!b.dead && boss?.alive && hit(b, boss)) {
          boss.hp -= b.damage; b.dead = true; burst(b.x, b.y, 5)
          localScore += 20; setScore(localScore)
          if (boss.hp <= 0) { boss.alive = false; burst(boss.x + boss.w / 2, boss.y + boss.h / 2, 70); transition = 2.4; setTimeout(() => {}, 0) }
        }
      }

      if (player.invuln <= 0) {
        for (const e of enemies) if (e.hp > 0 && hit(player, e)) { e.hp = 0; damagePlayer(); break }
        for (const b of enemyBullets) if (!b.dead && hit(player, b)) { b.dead = true; damagePlayer(); break }
        if (boss?.alive && hit(player, boss)) damagePlayer()
      }

      for (const p of pickups) if (!p.dead && hit(player, p)) {
        p.dead = true
    ...
    function drawEnemy(e) {
      const colors = { grunt: '#9b3d3d', runner: '#b36b2f', turret: '#65717c', heavy: '#6d2f39' }
      ctx.fillStyle = colors[e.type]; ctx.fillRect(e.x, e.y, e.w, e.h)
      ctx.fillStyle = '#211d22'; ctx.fillRect(e.x + 7, e.y + 7, Math.max(16, e.w - 14), 11)
      if (e.type !== 'turret') { ctx.fillStyle = '#b89a72'; ctx.fillRect(e.x + 10, e.y + 10, 3, 3); ctx.fillRect(e.x + e.w - 13, e.y + 10, 3, 3) }
      ctx.fillStyle = '#20262a'; ctx.fillRect(e.x - 10, e.y + Math.min(28, e.h - 10), 14, 5)
    }

    function drawBoss() {
      if (!boss?.alive) return
      ctx.fillStyle = levelIndex % 2 ? '#6f495d' : '#59636e'; ctx.fillRect(boss.x, boss.y, boss.w, boss.h)
      ctx.fillStyle = '#20242a'; ctx.fillRect(boss.x + 13, boss.y + 13, boss.w - 26, 32)
      ctx.fillStyle = '#d84f4f'; ctx.fillRect(boss.x + 27, boss.y + 22, 12, 10); ctx.fillRect(boss.x + boss.w - 39, boss.y + 22, 12, 10)
      ctx.fillStyle = current.accent; ctx.fillRect(boss.x - 26, boss.y + 62, 38, 16); ctx.fillRect(boss.x + boss.w - 12, boss.y + 62, 38, 16)
      ctx.fillStyle = '#16191d'; ctx.fillRect(boss.x + 20, boss.y + boss.h - 20, 28, 20); ctx.fillRect(boss.x + boss.w - 48, boss.y + boss.h - 20, 28, 20)
    }

    function drawBackground() {
      ctx.fillStyle = current.sky; ctx.fillRect(0, 0, W, H)
      ctx.fillStyle = current.far
      for (let i = 0; i < 11; i++) { const x = ((i * 180 - camera * .14) % 1260 + 1260) % 1260 - 100; ctx.fillRect(x, 175 - (i % 4) * 22, 125, 293) }
      ctx.fillStyle = current.near
      for (let i = 0; i < 17; i++) { const x = ((i * 105 - camera * .33) % 1160 + 1160) % 1160 - 80; ctx.fillRect(x, 300 - (i % 5) * 17, 68, 168) }
      if (levelIndex === 1) { ctx.fillStyle = '#103b27'; for (let i = 0; i < 10; i++) ctx.fillRect(((i * 150 - camera * .5) % 1100 + 1100) % 1100, 345, 16, 123) }
      if (levelIndex === 2) { ctx.fillStyle = '#d1a65d'; ctx.beginPath(); ctx.arc(790, 105, 52, 0, Math.PI * 2); ctx.fill() }
      if (levelIndex === 3) { ctx.fillStyle = '#7ea0b3'; for (let x = -50; x < W + 100; x += 180) ctx.fillRect(x - (camera * .6 % 180), 120, 4, 348) }
      ctx.fillStyle = current.ground; ctx.fillRect(0, GROUND, W, H - GROUND)
    }
    ...
    function draw() {
      ...
      ctx.fillStyle = '#ff6b5f'; enemyBullets.forEach(b => ctx.fillRect(b.x, b.y, b.w, b.h))
      enemies.forEach(drawEnemy)
      drawBoss()
      pickups.forEach(p => { ctx.fillStyle = p.type === 'life' ? '#78e08f' : '#ffd86b'; ctx.fillRect(p.x, p.y, p.w, p.h); ctx.fillStyle = '#17222b'; ctx.font = 'bold 14px monospace'; ctx.fillText(p.type === 'life' ? '+' : '$', p.x + 6, p.y + 16) })
      particles.forEach(p => { ctx.globalAlpha = Math.max(0, p.t * 1.4); ctx.fillStyle = '#ffb347'; ctx.fillRect(p.x, p.y, 5, 5) }); ctx.globalAlpha = 1
      ctx.restore()

      ctx.fillStyle = '#fff'; ctx.font = '700 18px monospace'; ctx.fillText(`SCORE ${String(localScore).padStart(6, '0')}`, 24, 31); ctx.fillText(`LIVES ${localLives}`, W - 132, 31)
      ctx.fillStyle = '#9fd3ff'; ctx.font = '700 15px monospace'; ctx.fillText(`LEVEL ${levelIndex + 1}/6  ${current.name.toUpperCase()}`, 24, 56)
      const progress = clamp((player.x / current.length), 0, 1)
      ctx.fillStyle = '#17232e'; ctx.fillRect(24, 68, 240, 7); ctx.fillStyle = current.accent; ctx.fillRect(24, 68, 240 * progress, 7)
      if (boss?.alive) {
        ctx.fillStyle = '#fff'; ctx.font = '700 14px monospace'; ctx.fillText(current.boss.toUpperCase(), 330, 31)
        ctx.fillStyle = '#351317'; ctx.fillRect(330, 42, 300, 12); ctx.fillStyle = '#e85d5d'; ctx.fillRect(330, 42, 300 * Math.max(0, boss.hp / boss.maxHp), 12)
      }
      ...
      if (transition > 0) {
        ...
        ctx.textAlign = 'center'; ctx.fillStyle = '#fff'; ctx.font = '900 34px monospace'; ctx.fillText(boss && !boss.alive ? 'TARGET DESTROYED' : `LEVEL ${levelIndex + 1}: ${current.name.toUpperCase()}`, W / 2, H / 2)
        ctx.font = '700 17px monospace'; ctx.fillStyle = '#9fd3ff'; ctx.fillText(boss && !boss.alive ? 'Advancing to the next operation...' : current.boss.toUpperCase() + ' AWAITS', W / 2, H / 2 + 34); ctx.textAlign = 'left'
      }
    }

    function loop(now) {
      const dt = Math.min((now - last) / 1000, .033); last = now
      update(dt); draw(); if (started) raf = requestAnimationFrame(loop)
    }
  ...
  const begin = () => {
    ...
    setScore(0); setLives(3); setLevel(1); setStatus('playing'); setStarted(true)
  }
