...
export default function GameShell(){
  ...
  useEffect(()=>{
    ...
    function spawnEnemy(type,x){const e={type,x,y:420,w:38,h:48,hp:2,vx:-70,cooldown:.6+Math.random(),bob:Math.random()*6};if(type==='runner')Object.assign(e,{hp:1,vx:-155,w:34,h:42,y:426});if(type==='turret')Object.assign(e,{hp:4,vx:0,w:42,h:36,y:432,cooldown:.5});if(type==='heavy')Object.assign(e,{hp:7,vx:-42,w:52,h:58,y:410,cooldown:1});enemies.push(e)}
    function spawnWave(mark){const x=mark+720,d=levelIndex+1;spawnEnemy('grunt',x);if(d>=2||mark>current.length*.3)spawnEnemy('runner',x+180);if(mark%900<250)spawnEnemy('turret',x+330);if(d>=3&&mark>current.length*.45)spawnEnemy('heavy',x+520);if(d>=5)spawnEnemy(Math.random()>.5?'runner':'grunt',x+700)}
    ...
    function nextLevel(){localScore+=1500+levelIndex*500;setScore(localScore);if(levelIndex===LEVELS.length-1){setStatus('victory');setStarted(false);saveScore(localScore);audio.victory();return}levelIndex++;current=LEVELS[levelIndex];setLevel(levelIndex+1);camera=0;checkpoint=0;waveCursor=0;bossSpawned=false;boss=null;bullets.splice(0);enemyBullets.splice(0);enemies.splice(0);pickups.splice(0);player.x=120;player.y=GROUND-player.h;player.vx=0;player.vy=0;player.grounded=true;player.invuln=1.4;transition=2.2;audio.level()}
    function update(dt){
    ...
    function drawHuman(x,y,f,kind='player',scale=1){ctx.save();ctx.translate(x,y);ctx.scale(f*scale,scale);const enemy=kind!=='player';const skin=enemy?'#a9795f':'#c18b6c',hair=enemy?'#2b211e':'#984b29',armor=enemy?(kind==='heavy'?'#4c4248':kind==='runner'?'#674a32':'#44523f'):'#344936';ctx.fillStyle='#171c20';ctx.fillRect(-15,57,11,16);ctx.fillRect(4,57,11,16);ctx.fillStyle='#2d3930';ctx.fillRect(-17,72,14,4);ctx.fillRect(3,72,15,4);ctx.fillStyle='#26372d';ctx.fillRect(-17,42,34,20);ctx.fillStyle=armor;ctx.fillRect(-20,24,40,25);ctx.fillStyle='#697354';ctx.fillRect(-15,28,30,7);ctx.fillStyle='#1c2925';ctx.fillRect(-18,38,36,8);ctx.fillStyle=skin;ctx.fillRect(-6,19,12,9);ctx.fillStyle='#d8d4c7';ctx.fillRect(-12,20,24,7);ctx.fillStyle=skin;ctx.fillRect(-11,6,22,17);ctx.fillStyle=hair;ctx.fillRect(-13,3,26,9);ctx.fillRect(-10,1,20,5);if(!enemy){ctx.fillStyle='#813b25';ctx.fillRect(-10,16,20,10);ctx.fillRect(-7,23,14,5)}ctx.fillStyle='#171a1c';ctx.fillRect(-6,13,3,2);ctx.fillRect(3,13,3,2);ctx.fillStyle='#3c503d';ctx.fillRect(9,28,12,18);ctx.fillRect(-15,29,11,17);ctx.fillStyle='#171b1d';ctx.fillRect(8,31,37,6);ctx.fillRect(41,29,9,4);ctx.fillStyle='#8f6b55';ctx.fillRect(18,40,8,6);ctx.fillStyle=enemy?'#a93b35':'#b63c35';ctx.fillRect(-20,27,5,8);ctx.restore()}
    function drawEnemy(e){if(e.type==='turret'){ctx.fillStyle='#65717c';ctx.fillRect(e.x,e.y+8,e.w,e.h-8);ctx.fillStyle='#252b31';ctx.fillRect(e.x+7,e.y,e.w-14,12);ctx.fillStyle='#c94a4a';ctx.fillRect(e.x+e.w-3,e.y+12,10,5);return}drawHuman(e.x+e.w/2,e.y,1,e.type,e.type==='heavy'?1.08:1)}
